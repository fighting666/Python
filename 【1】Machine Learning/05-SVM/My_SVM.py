    ...
    while (iter < maxIter):  # 循环直到超出最大迭代次数
        ...
        for i in range(m):
            ...
            if ((labelMat[i] * Ei < -toler) and (alphas[i] < C)) or \
                ...
                # 即Delta(alpha[i]) * labelMal[i] + Delta(alpha[j]) *
                # labelMal[j] = 0
                alphas[i] += labelMat[j] * \
...
def smoP(dataMatIn, classLabels, C, toler, maxIter, kTup=('lin', 0)):
    oS = optStruct(mat(dataMatIn), mat(
        classLabels).transpose(), C, toler, kTup)  # 建立数据结构
    iter = 0  # 一次迭代完成一次循环过程
    entireSet = True
    alphaPairsChanged = 0
        ...
        if entireSet:  # 判断1
            ...
            for i in nonBoundIs:
                ...
                print "non-bound, iter: %d i: %d, pairs changed %d" % \
                      (iter, i, alphaPairsChanged)
            iter += 1
        # 执行判断1时，如果entireSet = True，表示遍历整个集合，alphaPairsChanged =
        # 0，表示未对任意alpha对进行修改
        if entireSet:
            entireSet = False
...
def testRbf(k1=1.3):
    ...
    for i in range(m):
        ...
        predict = kernelEval.T * \
            multiply(labelSV, alphas[svInd]) + b  # 得验证集预测值
        if sign(predict) != sign(labelArr[i]):
            errorCount += 1
